...
namespace LaraWelP\Options\OptionsFieldGenerator;

use InvalidArgumentException;
use LaraWelP\Contracts\Options\OptionsRepository as OptionsRepositoryContract;
use LaraWelP\Contracts\Options\OptionsFieldGenerator as OptionsFieldGeneratorContract;
use LaraWelP\Options\OptionsPage;
use LaraWelP\Support\Template;
...
abstract class BaseFieldGenerator implements OptionsFieldGeneratorContract
{
    ...
    protected function generateHelpLink()
    {
        ...
        if ($helpLink = $this->config('helpLink')) {
            $markUp = "<a href=\"{$helpLink}\" target=\"_blank\" class=\"larawelp-help-link\"><i class=\"dashicons-before dashicons-info\"></i></a>";
        }
    ...
    protected function generateInput($type, $allAttributes = null)
    {
        ...
        if ($suffix or $prefix) {
            $suffix = $suffix ? "<span class=\"larawelp-input-group-addon\">{$suffix}</span>" : '';
            $prefix = $prefix ? "<span class=\"larawelp-input-group-addon\">{$prefix}</span>" : '';
            $html = "<div class=\"larawelp-input-group\">{$prefix}" . $html . "{$suffix}</div>";
        }

        // Add help link if possible
        if ($helpLink = $this->generateHelpLink()) {
            $html = '<div class="larawelp-flex-line">' . $html . "{$helpLink}</div>";
        }
